  ...
  public byte[] createResponse(InputStream inStream) {
    ...
    try {
	  ...
	  if (query_pairs.get("num1") == null || query_pairs.get("num2") == null) {
		...
		builder.append("Content-Type: text/html; charset=utf-8\n");
		builder.append("\n");
		builder.append("Use parameters num1 and num2.");
	  } else {
		  try {
	 ...
	 // JSONObject j = (JSONObject) parser.parse(json);
	  ...
	  for (int i = 0; i < j.length(); i++) {
		  System.out.println(j.getJSONObject(i).getString("name")
				  + ", " +
				  j.getJSONObject(i).getJSONObject(j.getJSONObject(i).getString("owner")).getString("login")
				  + " -> " +
				  j.getJSONObject(i).getJSONObject(j.getJSONObject(i).getString("owner")).getString("id"));
	  }	
